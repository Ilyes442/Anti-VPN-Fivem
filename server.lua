AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
    ...
    deferrals.defer()
    Wait(0)
    deferrals.update(string.format("Bonjour %s. Votre adresse IP est en cours de vérification.", name))
        ...
        PerformHttpRequest("http://ip-api.com/json/" .. ipIdentifier .. "?fields=proxy", function(err, text, headers)
            if tonumber(err) == 200 then
                ...
                if tbl["proxy"] == false then
                    ...
                    deferrals.done("Vous utilisez un VPN. Veuillez désactiver et réessayer.")
                end
            else
                deferrals.done("Il y avait une erreur dans l’API.")
            end
        end)
    end
end)

print('Created BY ILYES4')